	...
	function init()
	{
	    $.getJSON('/s/travels.json', 
			  ...
			  while (data[i])
			  {
			      ...
			      d = data[i].from.split(',');
			      a = data[i].to.split(',');
			      html += "<tr><th>"+data[i].from+"</th><th>"+data[i].to+"</th><th>"+Math.round(data[i].distance/1000)+"KM</th><th><a href='/timeline/?d="+d[0]+"&a="+a[0]+"&dist="+Math.round(data[i].distance/1000)+"&t="+Math.round(distance/1000)+"'>map</a></th><th><button class=\"btn btn-danger delete-btn\">delete</button></th></tr>";
			      i++;
			  }
			  $(".result").append(html);
